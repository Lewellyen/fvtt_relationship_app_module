...
| CA-02 | Performance | `JournalVisibilityService.getHiddenJournalEntries()` nutzt jetzt `CacheService` + Hook-basierte Invalidierung; Render-N+1 entfällt. | ✅ Gelöst (v0.22.0) | — |
...
2. **Hidden-Entry Cache (CA-02)** – ✅ Erledigt  
   - CacheService + JournalCacheInvalidationHook implementiert.  
   - Dokumentation (PROJECT-ANALYSIS, DEPENDENCY-MAP, CHANGELOG, CONFIGURATION) aktualisiert.
