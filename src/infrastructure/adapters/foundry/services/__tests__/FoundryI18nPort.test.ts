...
describe("FoundryI18nPort", () => {
  ...
  describe("dispose", () => {
    ...
    it("should clear cache even when port does not implement Disposable", () => {
      // Create a port that doesn't implement Disposable
      const nonDisposablePort: FoundryI18n = {
        localize: vi.fn().mockReturnValue(ok("Translated")),
        format: vi.fn().mockReturnValue(ok("Formatted")),
        has: vi.fn().mockReturnValue(ok(true)),
        dispose: undefined as any, // Explicitly set to undefined to test non-disposable path
      };
      ...
      const testService = new FoundryI18nPort(testSelector, mockRegistry, mockRetryService);
      testService.localize("TEST.KEY");
