...
describe("CompositionRoot", () => {
  ...
  describe("bootstrap", () => {
    ...
    it("should call onComplete callback when performance tracking is enabled and sampling passes", async () => {
      ...
      const _runtimeConfigModule = await import("@/core/runtime-config/runtime-config.service");
      ...
      const { CompositionRoot: compositionRootClass } = await import("../composition-root");
    ...
    it("should handle case when logger is not available in onComplete callback", async () => {
      ...
      const { loggerToken: testLoggerToken } = await import("@/tokens/tokenindex");
      ...
      const { ServiceContainer: serviceContainer } = await import("@/di_infrastructure/container");
